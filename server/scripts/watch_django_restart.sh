...
docker events \
  ...
  --format '{{.Time}}' | \
while read -r ts; do
    log "Django restarted (ts=$ts) — waiting 45s for it to initialize"
    sleep 45

    log "Logging agents back into Asterisk queues"
...
" >> /var/log/django_restart_watcher.log 2>&1 && log "Agents logged back in OK" || log "ERROR logging agents back in"
done

log "docker events ended — the watcher should be restarted by check_dashboard.sh"
